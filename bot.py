...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_test":
        try:
            msg = "Тестовый пост — бот работает! Канал подключен успешно"
        ...
        await admin_panel_from_callback(query)
        return
...
async def admin_panel_from_callback(query):
    total = len(all_users)
    ...
    if not all_users:
        ...
        for uid, u in list(all_users.items())[:30]:
            ...
            text += f"[{status}] {u['tg_name']} | @{u.get('username','—')} | {name}\n"
    keyboard = InlineKeyboardMarkup([
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not all_users:
        ...
        for uid, u in list(all_users.items())[:30]:
            ...
            joined = u.get("joined", "—")
            text += f"[{status}] {u['tg_name']} | @{username} | {name} | {joined}\n"
        if len(all_users) > 30:
...
async def test_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id != ADMIN_ID:
        await update.message.reply_text("Нет доступа.")
        return
    await update.message.reply_text("Отправляю тестовый пост...")
    try:
        msg = (
            "Тестовый пост — бот работает! Канал подключен успешно"
        )
        await context.bot.send_message(chat_id=CHANNEL_ID, text=msg)
